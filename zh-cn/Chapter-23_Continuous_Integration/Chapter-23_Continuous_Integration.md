...
在實踐中，"頻繁地整合工作 "對於現代的、分散式的應用程式意味著什麼？今天的系統除了儲存函式庫中最新版本的程式碼外，還有許多可移動的部分。事實上，隨著近來的微服務趨勢，破壞應用程式的變化不太可能存在於專案的即時程式碼函式庫中，而更可能存在於網路呼叫的另一端的鬆散耦合的微服務中。傳統的持續建構是測試二進位制檔案的變更，而其延伸則是測試上游微服務的變化。依賴關係只是從你的函式呼叫棧轉移到HTTP請求或遠端過程呼叫（RPC）。
...
甚至在程式碼依賴關係之外，應用程式可能會定期接收資料或更新機器學習模型。它可能在不斷髮展的作業系統、執行時、雲託管服務和裝置上執行。它可能是位於不斷增長的平臺之上的功能，也可能是必須適應不斷增長的功能基礎的平臺。所有這些都應該被視為依賴關係，我們也應該致力於“持續整合”它們的變化。更復雜的是，這些變化的元件通常由我們團隊、組織或公司之外的開發人員擁有，並按照他們自己的時間表部署。
